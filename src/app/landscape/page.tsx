...
export default async function LandscapeOverviewPage() {
  const supabase = await createClient()

  // Fetch data
  const [tamAccountsRes, signalsRes, warmPathsRes, campaignsRes, gapDefsRes, pursuitsRes, intelSignalsRes] = await Promise.all([
    // Include all active statuses for TAM accounts
    supabase.from('tam_accounts').select('*').in('status', ['Qualified', 'Researching', 'Pursuing', 'Prospecting', 'New']).order('priority_score', { ascending: false }),
  ...
  // Calculate metrics
  const totalLandscape = tamAccounts.length
  // Count intelligence signals from AI scan (more relevant than old account_signals)
  const intelSignalsCount = intelligenceSignals.length
  const matchedSignalsCount = intelligenceSignals.filter(s => s.tam_account_id).length
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">Landscape</h1>
          <p className="text-zinc-500 dark:text-zinc-400 mt-1">Scan the landscape to find high-value targets</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        <div className="rounded-lg bg-white dark:bg-zinc-900 p-4 border border-zinc-200 dark:border-zinc-800">
          <p className="text-sm text-zinc-500 dark:text-zinc-400">Landscape Accounts</p>
          <p className="text-2xl font-bold text-zinc-900 dark:text-zinc-100 mt-1">{totalLandscape}</p>
        </div>
        <div className="rounded-lg bg-white dark:bg-zinc-900 p-4 border border-zinc-200 dark:border-zinc-800">
        ...
        <div className="rounded-lg bg-indigo-50 dark:bg-indigo-900/20 p-4 border border-indigo-200 dark:border-indigo-800">
          <p className="text-sm text-indigo-600 dark:text-indigo-400">Landscape Signals</p>
          <p className="text-2xl font-bold text-indigo-700 dark:text-indigo-300 mt-1">
            {intelSignalsCount}
            {matchedSignalsCount > 0 && (
              <span className="text-sm font-normal text-indigo-500 ml-2">({matchedSignalsCount} matched)</span>
            )}
          </p>
        </div>
        <div className="rounded-lg bg-white dark:bg-zinc-900 p-4 border border-zinc-200 dark:border-zinc-800">
        ...
        <div className="col-span-3 space-y-6">
          <section className="rounded-lg bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/landscape/list?min_priority=70" className="text-sm text-blue-600 hover:text-blue-700">
                View All →
              </Link>
            </div>
            ...
            <div className="space-y-2">
              {priorityQueue.map((account) => (
                <Link
                  ...
                  href={`/landscape/${account.tam_account_id}`}
              ...
              {priorityQueue.length === 0 && (
                <p className="text-zinc-500 dark:text-zinc-400 text-sm text-center py-8">No landscape accounts yet</p>
              )}
            </div>
          </section>
        </div>
                ...
                {gapAlerts.map((gap) => (
                  <div key={gap.id} className="p-3 bg-white dark:bg-zinc-900 rounded-lg">
                    ...
                    <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-1">
                      {gap.pursuitCount} pursuits • {gap.tamCount} landscape accounts • {gap.highPriorityCount} high priority
                    </p>
                    <Link
                      href={`/landscape/list?vertical=${gap.vertical || ''}&min_value=${gap.value_min || ''}`}
            ...
            <section className="rounded-lg bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 p-5">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-indigo-800 dark:text-indigo-200">
                  Landscape Signals ({intelligenceSignals.length})
                </h2>
                ...
                {intelligenceSignals.slice(0, 10).map((signal) => (
                  ...
                  >
                    ...
                    {signal.tam_account_id && signal.tam_accounts && (
                      <Link
                        href={`/landscape/${signal.tam_account_id}`}
                    ...
                    {signal.company_mentioned && !signal.tam_account_id && (
                      <p className="text-xs text-zinc-500 mt-2">
                        Company: {signal.company_mentioned} (not in Landscape)
                      </p>
                    )}
                  </div>
                ))}
            ...
            <div className="space-y-2">
              {campaigns.map((campaign) => (
                <Link
                  ...
                  href={`/landscape/list?campaign=${campaign.campaign_id}`}
