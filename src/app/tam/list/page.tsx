...
export default async function TAMListPage({
  searchParams,
}: {
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">TAM List</h1>
          <p className="text-zinc-500 dark:text-zinc-400 mt-1">{tamAccounts.length} accounts</p>
        </div>
            ...
            {tamAccounts.map((account) => (
              <tr key={account.tam_account_id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                <td className="px-4 py-3">
                  <Link href={`/tam/${account.tam_account_id}`} className="font-medium text-zinc-900 dark:text-zinc-100 hover:text-blue-600">
            ...
            {tamAccounts.length === 0 && (
              <tr>
                <td colSpan={9} className="px-4 py-8 text-center text-zinc-500 dark:text-zinc-400">
                  No TAM accounts match your filters
                </td>
              </tr>
            )}
          </tbody>
